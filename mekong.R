...
gms_rivers <- readShapeLines("data/gms_rivers/gms_river.shp")
gms_dams <- readOGR("data/mekongdamsgms/mekongdams_gms.shp")


dams_df <- fortify(gms_dams)
country <- readShapeLines("data/country/World_Countries__Generalized_.shp")
rivers <- readShapeLines("data/rivers/ec7a4add-5ef0-406f-845e-07ba64f55151202041-1-sx4gtf.f7uk.shp")
png("rivers.png", width=960, height=700)
par(mar=c(0,0,0,0))
plot(0,0, type="n", axes=FALSE, xlim=c(85, 115), ylim=c(5, 35), xlab=NA, ylab=NA)
lines(country, lwd=1)
lines(rivers, col="blue", lwd=2.5)
lines(gms_rivers, col="#008899", lwd=0.5)
dev.off()
